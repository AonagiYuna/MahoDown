...
- **语法** — GFM 表格与任务列表、**Mermaid 图**、**脚注**、**定义列表**；分屏 / 打印可渲染安全 HTML（脚本等会被去掉）
...
**Markdown**　分屏预览与打印支持脚注（`[^1]`）、定义列表（`术语` + `: 解释`）、以及常见 HTML（如 `<br>`、`<kbd>`、`<details>`）。Mermaid 图可点击放大、滚轮缩放。富文本以所见即所得为主，复杂图与 HTML 请看分屏。
